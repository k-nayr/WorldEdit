...
public class ConfigurateConfiguration extends LocalConfiguration {
    ...
    public void load() {
        ...
        wandItem = node.node("wand-item").getString(wandItem);
        try {
        ...
        try {
            disallowedBlocks = new HashSet<>(node.node("limits", "disallowed-blocks").getList(TypeToken.get(String.class)));
        } catch (SerializationException e) {
            logger.warn("Error loading WorldEdit configuration", e);
        }
        try {
            allowedDataCycleBlocks = new HashSet<>(node.node("limits", "allowed-data-cycle-blocks").getList(TypeToken.get(String.class)));
        } catch (SerializationException e) {
            logger.warn("Error loading WorldEdit configuration", e);
        }
        ...
        navigationWand = node.node("navigation-wand", "item").getString(navigationWand);
        try {
